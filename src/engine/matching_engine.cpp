...
MatchingEngine::tryToMatchReceivedOrder(core::Order &receivedOrder,
  ...
  while (receivedOrder.unfilledQty > 0) {
    ...
    if (receivedOrder.type == core::OrderType::Limit &&
        comparePrices(receivedOrder.price, bestBookOrder.price))
      break;
...
core::Trade MatchingEngine::matchOrders(core::Order &freshOrder,
  ...
  const core::Quantity tradeQty =
      std::min(freshOrder.unfilledQty, bestExistingOrder.unfilledQty);
  freshOrder.unfilledQty -= tradeQty;

  LOG_DEBUG(std::format(
      "Matched {} order {} with open {} order {}, for quantity {}, at price {}",
  ...
  const core::Trade trade{tradePrice, tradeQty, bestExistingOrder.id,
                          freshOrder.id};

  if (freshOrder.unfilledQty == 0) {
    LOG_INFO(std::format("Fully filled {} order {}.", receivedOrderSide,
                         freshOrder.id));
  }
...
void MatchingEngine::handlePartiallyFilledOrder(
    const core::Order &receivedOrder) {
  if (receivedOrder.unfilledQty > 0) {
    LOG_DEBUG(std::format("Unfilled quantity for order {}: {}",
                          std::to_string(receivedOrder.id),
                          std::to_string(receivedOrder.unfilledQty)));
    if (receivedOrder.type == core::OrderType::Market) {
      LOG_DEBUG(
          std::format("Partially filled market order {}, remaining unfilled "
                      "quantity: {}, canceling remaining quantity",
                      std::to_string(receivedOrder.id),
                      std::to_string(receivedOrder.unfilledQty)));
